...
const CategoryList = () => {
	...
	return (
		<div>
			...
			<Breadcrumb data={[{ name: 'Category', class: '' }, { name: 'Manage', class: 'is-active' }]} />
			<NavLink to="/category/create" className={'button is-info mb-4'}><IoAddCircleOutline size={25} className="mx-1" /> Create Category</NavLink>
						...
						{categories.map((category, index) => (
							<tr key={category.id}>
								...
								<td className='is-flex is-justify-content-center is-align-content-center ' >
									<NavLink to={`/category/${category.id}`} className="button is-info mx-4">Edit</NavLink>
