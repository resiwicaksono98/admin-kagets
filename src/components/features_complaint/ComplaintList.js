...
const ComplaintList = () => {
   ...
   const handleDetail = async (id) => {
      try {
         ...
         const { data } = await requestApi.get(`/complaints/${id}`);
         setDataById(data.data);
      } catch (error) {}
   };
   ...
   const handleDelete = (id) => {
      ...
      const deleteAction = async () => {
         ...
         try {
            await requestApi.delete(`/complaints/${id}`);
   ...
   const getComplaint = async () => {
      try {
         const { data } = await requestApi.get(`/complaints`);
                  ...
                  {complaints.map((complaint, index) => (
                     <tr key={complaint.id}>
                        ...
                        <td className="has-text-centered ">
                           <strong>{complaint.complaint_result?.estimated_time ? complaint.complaint_result.estimated_time + "Days" : "belum di tentukan"}</strong>
                        </td>
                        <td className="is-flex is-justify-content-center is-align-content-center ">
                           <button className="button is-success" onClick={() => handleDetail(complaint.id)}>
                              Detail
                           </button>
                           <button className="button is-link ml-4" onClick={() => handleStatus(complaint.complaint_result.id)}>
                              Change Status Instant
                           </button>
                           <NavLink to={`/complaint_result/${complaint.complaint_result.id}`} className="button is-info mx-4">
                              Edit And Send Message
                           </NavLink>
                           <button className="button is-danger" onClick={() => handleDelete(complaint.id)}>
                              Delete
                           </button>
                        </td>
                     </tr>
