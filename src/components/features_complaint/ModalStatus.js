...
const ModalStatus = ({ isShowing, hide, data }) => {
   ...
   const handleQuickStatus = async (e) => {
      ...
      try {
         const changeStatus = await requestApi.put(`/complaint_result/${data.id}`, { status: e });
            ...
            <div className="modal-card">
               ...
               <section className="modal-card-body">
                  <ul className="">
                     ...
                     <li className="py-2 px-3">
                        Estimated time: <strong>{data?.estimated_time}</strong> Hari
                     </li>
                     <li className="py-2 px-3 flex">Created At : {moment(data?.createdAt).format("h:mm:A - D MMM YYYY")} </li>
                  </ul>
               </section>
               <footer className="modal-card-foot">
                  <button className="button is-success" name="success" onClick={(e) => handleQuickStatus(e.target.name)}>
                     Success
                  </button>
                  <button className="button is-dark" name="pending" onClick={(e) => handleQuickStatus(e.target.name)}>
                     Pending
                  </button>
                  <button className="button is-warning" name="checked" onClick={(e) => handleQuickStatus(e.target.name)}>
                     Checked
                  </button>
                  <button className="button is-danger" name="failed" onClick={(e) => handleQuickStatus(e.target.name)}>
                     Failed
                  </button>
               </footer>
            </div>
