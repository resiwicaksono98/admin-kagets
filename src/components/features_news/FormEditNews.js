...
const FormEditNews = () => {
	...
	useEffect(() => {
		...
		if (!isUnmount) {
			const getNews = async () => {
				try {
					const { data } = await axios.get(`http://localhost:5000/api/news/${id}`)
					formik.values.title = data.data.title
					formik.values.categoryId = data.data.categoryId
					formik.values.description = data.data.description
					formik.values.rate = data.data.rate
					formik.values.previewImage = `http://localhost:5000/` + data.data.image
				} catch (error) {
					console.log(error);
				}
	...
	const formik = useFormik({
		...
		onSubmit: async (value) => {
			...
			const updateHandler = async () => {
				...
				try {
					await axios.patch(`http://localhost:5000/api/news/${id}`, value, {
						headers: {
		...
		<div>
			...
			<form onSubmit={formik.handleSubmit}>
				...
				<div className='columns'>
					...
					<div className="field column is-6">
						<div className='mb-4'>
							{formik.values.previewImage ? <img src={formik.values.previewImage} alt="preview" /> : <div className="mt-5" >No Image Preview</div>}
						</div>
					</div>
