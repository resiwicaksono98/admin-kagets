...
const NewsList = ({ newst }) => {
	...
	const handleDelete = async (e) => {
		...
		const nextDelete = async () => {
			...
			try {
				await axios.delete(`http://localhost:5000/api/news/${e.id}`)
	...
	return (
		<div className="container pb-8">
			...
			<Breadcrumb data={[{ name: 'News', class: '' }, { name: 'Manage', class: 'is-active' }]} />
			<Search />
			<NavLink to="/news/create" className={'button is-info mb-4'}><IoAddCircleOutline size={25} className="mx-1" /> Create News</NavLink>
						...
						<div className="card">
							<div className="card-image">
								<figure className="image is-4by3">
									<img src={'http://localhost:5000/' + news.image} alt="news/images" />
								</figure>
							</div>
							<div className="card-content">
								...
								<div className="content">
									...
									<time dateTime="2016-1-1"><strong>{moment(news.createdAt).format('h:mm:A - D MMM YYYY')}</strong></time>
								</div>
								<div>
									<button className="button is-danger" onClick={() => handleDelete({ id: news.id })}>Delete</button>
									<NavLink to={`/news/${news.id}`} className="button is-info mx-4">Edit</NavLink>
								</div>
							</div>
						</div>
