...
const FormAddProblem = () => {
	...
	const formik = useFormik({
		...
		onSubmit: async (value) => {
			ConfirmAlert({ title: 'Add Data', message: 'Lanjut Add Problem?', onClick: () => updateHandler() })
			const updateHandler = async () => {
				const loading = toast.loading("Please wait....")
				try {
					await axios.post(`http://localhost:5000/api/problem_type`, value,)
...
return (
	<div>
		<Breadcrumb data={[{ name: 'Problem', class: '' }, { name: 'Create', class: 'is-active' }]} />
