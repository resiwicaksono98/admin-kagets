...
const ProblemList = () => {
   ...
   const getCategory = async () => {
      try {
         const { data } = await requestApi.get(`/problem_type`);
   ...
   const handleDelete = async (id) => {
      ...
      const deleteAction = async () => {
         ...
         try {
            await requestApi.delete(`/problem_type/${id}`);
   ...
   return (
      <div>
         ...
         />
         <NavLink to="/problem/create" className={"button is-info mb-4"}>
            <IoAddCircleOutline size={25} className="mx-1" /> Create Problem
         </NavLink>
                  ...
                  {problems.map((problem, index) => (
                     <tr key={problem.id}>
                        ...
                        <td className="is-flex is-justify-content-center is-align-content-center ">
                           <NavLink to={`/problem/${problem.id}`} className="button is-info mx-4">
                              Edit
                           </NavLink>
                           <button className="button is-danger" onClick={() => handleDelete(problem.id)}>
                              Delete
                           </button>
                        </td>
                     </tr>
